...
def check_number_of_files_in_folder(path):
    return len(glob.glob(path + "/*")) # len([name for name in os.listdir(path + "/.") if os.path.isfile(name)])

def change_suffix(file, suffix):
    base = os.path.splitext(file)[0]
    return base+suffix

def make_yolo_data(data_path, train_dev_split, output_path):
    ...
    image_number = 0
    with open(os.path.join(output_path, "train.txt"), "w") as train, open(os.path.join(output_path ,"dev.txt"), "w") as dev:
        for file in os.listdir(image_path):
            if file.endswith(".png"):
                ...
                copyfile(os.path.join(image_path, file), output_images_path + "/" + str(image_number) + ".png")
                
                with open(os.path.join(output_labels_path, str(image_number) + ".txt"), "w") as label, \
                        open(os.path.join(image_path, change_suffix(file, ".txt")), "r") as label_unmodified:
